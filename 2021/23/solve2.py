# from https://gitlab.com/dos1/aoc21/-/blob/main/day23.py
...
def stoppable(data):
    return tuple(i for i in range(1,
                                  len(data[0]) - 1) if i not in rooms.values())
...
def roomsComplete(data):
    return isRoomComplete(data, 3) and isRoomComplete(
        data, 5) and isRoomComplete(data, 7) and isRoomComplete(data, 9)
...
def canMove(data, x, y):
    return (not isInOwnRoom(data, x, y)
            or isBlockingRoom(data, x, y)) and not isBlockedInRoom(data, x, y)
...
def move(d, x, y, i, j):
    newData = (*((*(((field(d, a, b), field(
        d, x, y))[a == i and b == j], field(d, i, j))[a == x and b == y]
                    for a in range(len(d[b]))), ) for b in range(len(d))), )
