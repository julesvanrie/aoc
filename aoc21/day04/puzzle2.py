...
dim = 5
lines = [[int(el) for el in line.split(' ') if el != ''] for line in lines
         if line != '']

boards = [lines[i:i + dim] for i in range(0, len(lines), dim)]
nb_boards = len(boards)

results = [[[1 for _ in range(dim)] for _ in range(dim)]
           for _ in range(nb_boards)]
...
for r in range(dim):
    for c in range(dim):
        score += boards[winners[-1]][r][c] * results[winners[-1]][r][c]
score *= win_number
